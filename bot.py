...
# 🔥 CUSTOM LINK (Caption Click hone par ye khulega)
CUSTOM_LINK = "https://t.me/+6UzDZ4URQWYzNTVl"

# Force Sub Channel Config
FORCE_CHANNEL = "@tmkocdirect"  # Main Channel Username
FORCE_CHANNEL_LINK = "https://t.me/+6UzDZ4URQWYzNTVl" # Join Link
...
def get_file_size(msg):
    try:
        if msg.video:
            return msg.video.file_size / (1024 * 1024)
        elif msg.document:
            return msg.document.file_size / (1024 * 1024)
    except:
        return 0
    return 0

# ================= CORE LOGIC (SEND EPISODE) =================
# Ye logic alag function me banaya taaki Buttons aur Text dono se call ho sake
async def send_episode_logic(update: Update, context: ContextTypes.DEFAULT_TYPE, ep_num: int):
    ...
    if ep_num < 4600:
        search_query = f"Taarak Mehta Ka Ooltah Chashmah Episode {ep_num}".replace(" ", "+")
    ...
    if not data:
        ...
        await message.reply_text(
            f"❌ *Episode {ep_num} Not Found*\n\nAgar ye naya episode hai, to Request karein 👇",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return
        ...
        for r in data:
            if r[1] == q:
                try:
                    ...
                    link_text = m.caption if m.caption else f"TMKOC Episode {ep_num} {q}"
                    
                    # Custom Caption
                    new_caption = (
                        f"<b><a href='{CUSTOM_LINK}'>{link_text}</a></b>\n"
...
async def latest_episodes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Get all column A (Episode numbers)
        all_eps = sheet.col_values(1)
        # Remove header and reverse list
        if len(all_eps) > 1:
            all_eps = all_eps[1:]
            
            # Get Unique Episodes (Preserve Order from bottom)
            seen = set()
            unique_eps = []
            for ep in reversed(all_eps):
                if ep not in seen and ep.isdigit():
                    ...
                    if len(unique_eps) >= 6: # Top 6 episodes
                        break
            
            if not unique_eps:
                await processing.edit_text("❌ No episodes found in database.")
                return
            ...
            await processing.edit_text("❌ Database Empty.")
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        total_files = len(sheet.col_values(1)) - 1
    except:
        total_files = "Error"
...
async def get_episode_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Call the logic
    await send_episode_logic(update, context, ep_num)
...
def main():
    ...
    app.add_handler(CommandHandler("latest", latest_episodes)) # ✅ New Command
    ...
    print("TMKOC Bot Running... (Admin Stats + Latest + Maintenance)")
